...
public extension Thenable {
    ...
    func flatMap<U>(on: DispatchQueue? = conf.Q.mapView, _ transform: @escaping(T) throws -> U?) -> Promise<U> {
...
public extension Thenable where T: Sequence {
    ...
    func map<U>(on: DispatchQueue? = conf.Q.mapView, _ transform: @escaping(T.Iterator.Element) throws -> U) -> Promise<[U]> {
        return mapValues(on: on, transform)
    }

    /// disabled due to ambiguity with the other `.flatMap`
    @available(*, deprecated, message: "See: `flatMapValues`")
    func flatMap<U: Sequence>(on: DispatchQueue? = conf.Q.mapView, _ transform: @escaping(T.Iterator.Element) throws -> U) -> Promise<[U.Iterator.Element]> {
